...
NacosString IOUtils::readStringFromFile(const NacosString &file,
...
void IOUtils::writeStringToFile(const NacosString &file,
                                const NacosString &data,
                                const NacosString &encoding)
    NACOS_THROW(IOException) {
...
bool IOUtils::recursivelyRemove(const NacosString &file) {
  ...
  if (stat(file.c_str(), &thestat) == -1 && errno != ENOENT) {
    // Something's wrong, and it's not "FileNotExist", we should record this and
    // exit
