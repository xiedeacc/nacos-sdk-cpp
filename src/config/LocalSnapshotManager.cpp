...
NacosString LocalSnapshotManager::getFailover(const NacosString &serverName,
  ...
  if (IOUtils::checkNotExistOrNotFile(localPath)) {
    log_debug("[LocalSnapshotManager]-getFailover:[servername=%s] failover "
  ...
  try {
    ...
    log_error("[LocalSnapshotManager]-getFailover:[servername=%s] get failover "
 ...
 */
NacosString LocalSnapshotManager::getSnapshot(const NacosString &name,
                                              const NacosString &dataId,
                                              const NacosString &group,
                                              const NacosString &tenant) {
  ...
  try {
    ...
    log_error("[LocalSnapshotManager]-getSnapshot:[servername=%s]+get snapshot "
              "error, file:%s what:%s\n",
...
NacosString LocalSnapshotManager::readFile(const NacosString &file)
...
void LocalSnapshotManager::saveSnapshot(const NacosString &envName,
                                        const NacosString &dataId,
                                        const NacosString &group,
                                        const NacosString &tenant,
  ...
  if (NacosisNull(config)) {
    ...
    if (remove_result) // error happens when removing the file
    {
      ...
      log_debug("[LocalSnapshotManager]-saveSnapshot:[servername=%s] delete "
                "snapshot error, remove() returns %d, errno = %d\n",
...
void LocalSnapshotManager::cleanAllSnapshot() {
  ...
  for (std::list<NacosString>::iterator it = rootFile.begin();
       it != rootFile.end(); it++) {
    // endsWith("_nacos")
...
void LocalSnapshotManager::cleanEnvSnapshot(const NacosString &envName) {
  ...
  tmp += "/snapshot";
  // I think we should remove -tenant also, so for one envname, cache for all
  // tenants within the environment will be purged
  NacosString tmp_tenant = tmp + "-tenant";
  IOUtils::cleanDirectory(tmp);
  IOUtils::cleanDirectory(tmp_tenant);
  log_info("[LocalSnapshotManager]-cleanEnvSnapshot:success delete "
           "%s-snapshot: %s\n",
           envName.c_str(), tmp.c_str());
};

NacosString LocalSnapshotManager::getFailoverFile(const NacosString &serverName,
                                                  const NacosString &dataId,
                                                  const NacosString &group,
                                                  const NacosString &tenant) {
...
NacosString LocalSnapshotManager::getSnapshotFile(const NacosString &envName,
                                                  const NacosString &dataId,
                                                  const NacosString &group,
                                                  const NacosString &tenant) {
...
LocalSnapshotManager::LocalSnapshotManager(AppConfigManager *appConfigManager) {
  ...
  log_debug("LocalConfigInfoProcessor::LocalConfigInfoProcessor() "
            "LOCAL_SNAPSHOT_PATH = %s\n",
