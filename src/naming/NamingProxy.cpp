...
NamingProxy::NamingProxy(ObjectConfigData *objectConfigData) {
  ...
  log_debug("NamingProxy Constructor:\n"
            ...
            objectConfigData->_serverListManager->getNamespace().c_str(),
            objectConfigData->_serverListManager->getEndpoint().c_str(),
            objectConfigData->_serverListManager->toString().c_str());
  serverPort = "8848";

  if (objectConfigData->_serverListManager->getServerCount() == 1) {
    nacosDomain = objectConfigData->_serverListManager->getServerList()
                      .begin()
                      ->getCompleteAddress();
  }
  ...
  _hb_fail_wait = atoi(objectConfigData->_appConfigManager
                           ->get(PropertyKeyConst::HB_FAIL_WAIT_TIME)
                           .c_str());
}

NamingProxy::~NamingProxy() {}

void NamingProxy::registerService(const NacosString &serviceName,
                                  const NacosString &groupName,
                                  Instance &instance)
    NACOS_THROW(NacosException) {
  log_info("[REGISTER-SERVICE] %s registering service %s with instance: %s\n",
           getNamespaceId().c_str(), serviceName.c_str(),
  ...
  ParamUtils::addKV(params, "enable",
                    NacosStringOps::valueOf(instance.enabled));
  ParamUtils::addKV(params, NamingConstant::HEALTHY,
                    NacosStringOps::valueOf(instance.healthy));
  ParamUtils::addKV(params, "ephemeral",
                    NacosStringOps::valueOf(instance.ephemeral));
  ParamUtils::addKV(params, "metadata", JSON::toJSONString(instance.metadata));

  reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
             UtilAndComs::NACOS_URL_INSTANCE,
         params, IHttpCli::POST);
}

void NamingProxy::deregisterService(const NacosString &serviceName,
                                    Instance &instance)
    NACOS_THROW(NacosException) {
  log_info(
      "[DEREGISTER-SERVICE] %s deregistering service %s with instance: %s\n",
      getNamespaceId().c_str(), serviceName.c_str(),
      instance.toString().c_str());
  ...
  reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
             UtilAndComs::NACOS_URL_INSTANCE,
         params, IHttpCli::DELETE);
}

NacosString NamingProxy::queryList(const NacosString &serviceName,
                                   const NacosString &groupName,
                                   const NacosString &clusters, int udpPort,
                                   bool healthyOnly)
    NACOS_THROW(NacosException) {
  list<NacosString> params;
  const NacosString &localIp =
      _objectConfigData->_appConfigManager->get(PropertyKeyConst::LOCAL_IP);
  ParamUtils::addKV(params, NamingConstant::NAMESPACE_ID, getNamespaceId());
  ParamUtils::addKV(params, NamingConstant::SERVICE_NAME, serviceName);
  ParamUtils::addKV(params, NamingConstant::GROUP_NAME, groupName);
  ParamUtils::addKV(params, NamingConstant::CLUSTERS, clusters);
  ParamUtils::addKV(params, NamingConstant::UDP_PORT,
                    NacosStringOps::valueOf(udpPort));
  ParamUtils::addKV(params, NamingConstant::CLIENT_IP, localIp);
  ParamUtils::addKV(params, NamingConstant::HEALTHY_ONLY,
                    NacosStringOps::valueOf(healthyOnly));

  return reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                    UtilAndComs::NACOS_URL_BASE + "/instance/list",
                params, IHttpCli::GET);
}

NacosString NamingProxy::reqAPI(const NacosString &api,
                                list<NacosString> &params, int method)
    NACOS_THROW(NacosException) {
  ...
  if (_serverListManager->getServerCount() == 0) {
    throw NacosException(NacosException::NO_SERVER_AVAILABLE,
                         "no server available");
  }
  ...
  if (!servers.empty()) {
    ...
    for (size_t i = 0; i < servers.size(); i++) {
      const NacosServerInfo &server =
          ParamUtils::getNthElem(servers, selectedServer);
    ...
    throw NacosException(NacosException::ALL_SERVERS_TRIED_AND_FAILED,
                         "failed to req API:" + api + " after all servers(" +
                             _serverListManager->toString() +
                             ") tried: " + errmsg);
  }
  ...
  for (int i = 0; i < UtilAndComs::REQUEST_DOMAIN_RETRY_COUNT; i++) {
    try {
      ...
      log_error("[NA] req api:%s failed, server(%s), e = %s\n", api.c_str(),
                nacosDomain.c_str(), e.what());
    }
  }

  throw NacosException(NacosException::ALL_SERVERS_TRIED_AND_FAILED,
                       "failed to req API:/api/" + api + " after all servers(" +
                           _serverListManager->toString() +
                           ") tried: " + errmsg);
}

NacosString NamingProxy::callServer(const NacosString &api,
                                    list<NacosString> &params,
...
NacosString
NamingProxy::getDataToSign(const std::list<NacosString> &paramValues,
                           NacosString &nowTimeMs) {
  const NacosString &serviceName =
      ParamUtils::findByKey(paramValues, NamingConstant::SERVICE_NAME);
...
NacosString NamingProxy::callServer(const NacosString &api,
                                    list<NacosString> &params,
                                    const NacosString &curServer, int method)
    NACOS_THROW(NacosException) {
  ...
  // SPAS security
  NacosString secretKey =
      _objectConfigData->_appConfigManager->get(PropertyKeyConst::SECRET_KEY);
  NacosString accessKey =
      _objectConfigData->_appConfigManager->get(PropertyKeyConst::ACCESS_KEY);

  // If SPAS security credentials are set, SPAS is enabled
  if (!ParamUtils::isBlank(secretKey) && !ParamUtils::isBlank(accessKey)) {
    NacosString nowTimeMs =
        NacosStringOps::valueOf(TimeUtils::getCurrentTimeInMs());
    NacosString dataToSign = getDataToSign(params, nowTimeMs);
    NacosString signature =
        SignatureTool::SignWithHMAC_SHA1(dataToSign, secretKey);
  ...
  try {
    ...
    case IHttpCli::GET:
      requestRes =
          _httpDelegate->httpGet(requestUrl, headers, params,
                                 UtilAndComs::ENCODING, _http_req_timeout);
      break;
    case IHttpCli::PUT:
      requestRes =
          _httpDelegate->httpPut(requestUrl, headers, params,
                                 UtilAndComs::ENCODING, _http_req_timeout);
      break;
    case IHttpCli::POST:
      requestRes =
          _httpDelegate->httpPost(requestUrl, headers, params,
                                  UtilAndComs::ENCODING, _http_req_timeout);
      break;
    case IHttpCli::DELETE:
      requestRes =
          _httpDelegate->httpDelete(requestUrl, headers, params,
                                    UtilAndComs::ENCODING, _http_req_timeout);
      break;
    }
  ...
  throw NacosException(requestRes.code,
                       "failed to req API:" + requestUrl +
                           " code:" + NacosStringOps::valueOf(requestRes.code) +
...
long NamingProxy::sendBeat(BeatInfo &beatInfo) {
  try {
    ...
    log_info("[BEAT] %s sending beat to server: %s\n", getNamespaceId().c_str(),
             beatInfoStr.c_str());
    list<NacosString> params;
    ParamUtils::addKV(params, NamingConstant::BEAT,
                      JSON::toJSONString(beatInfo));
    ParamUtils::addKV(params, NamingConstant::NAMESPACE_ID, getNamespaceId());
    ParamUtils::addKV(params, NamingConstant::SERVICE_NAME,
                      beatInfo.serviceName);
    NacosString result =
        reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                   UtilAndComs::NACOS_URL_BASE + "/instance/beat",
               params, IHttpCli::PUT);
    // JSONObject jsonObject = JSON.parseObject(result);

    if (!NacosisNull(result)) {
...
ListView<NacosString> NamingProxy::getServiceList(int page, int pageSize,
                                                  const NacosString &groupName)
    NACOS_THROW(NacosException) {
  log_debug("[NAMEPRXY] request:group=%s page=%d pageSize=%d\n",
            groupName.c_str(), page, pageSize);
  list<NacosString> params;
  ParamUtils::addKV(params, NamingConstant::PAGE_NO,
                    NacosStringOps::valueOf(page));
  ParamUtils::addKV(params, NamingConstant::PAGE_SIZE,
                    NacosStringOps::valueOf(pageSize));
  ParamUtils::addKV(params, NamingConstant::GROUP_NAME, groupName);
  ParamUtils::addKV(params, NamingConstant::NAMESPACE_ID, getNamespaceId());
  NacosString result =
      reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                 UtilAndComs::NACOS_URL_BASE + "/service/list",
             params, IHttpCli::GET);
...
ServiceInfo2 NamingProxy::getServiceInfo(const NacosString &serviceName,
                                         const NacosString &groupName)
    NACOS_THROW(NacosException) {
  ...
  if (!NacosStringOps::isNullStr(groupName)) {
    ...
    ParamUtils::addKV(params, NamingConstant::GROUP_NAME,
                      ConfigConstant::DEFAULT_GROUP);
  }
  ParamUtils::addKV(params, NamingConstant::NAMESPACE_ID, getNamespaceId());
  NacosString result =
      reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                 UtilAndComs::NACOS_URL_BASE + "/service",
             params, IHttpCli::GET);
 ...
 * please note that the operation will succeed ONLY WHEN there's NO instance of
 ...
 */
bool NamingProxy::deleteServiceInfo(const NacosString &serviceName,
                                    const NacosString &groupName)
    NACOS_THROW(NacosException) {
  list<NacosString> params;
  ParamUtils::addKV(params, NamingConstant::SERVICE_NAME, serviceName);
...
void assembleServiceInfoRequest(list<NacosString> &target,
                                const ServiceInfo2 &serviceInfo2) {
  if (serviceInfo2.isNameSet()) {
    ParamUtils::addKV(target, NamingConstant::SERVICE_NAME,
                      serviceInfo2.getName());
  }
  if (serviceInfo2.isGroupNameSet()) {
    ParamUtils::addKV(target, NamingConstant::GROUP_NAME,
                      serviceInfo2.getGroupName());
  }
  if (serviceInfo2.isNamespaceIdSet()) {
    ParamUtils::addKV(target, NamingConstant::NAMESPACE_ID,
                      serviceInfo2.getNamespaceId());
  }
  if (serviceInfo2.isProtectThresholdSet()) {
    ParamUtils::addKV(
        target, "protectThreshold",
        NacosStringOps::valueOf(serviceInfo2.getProtectThreshold()));
  }
  if (serviceInfo2.isMetadataSet()) {
    ParamUtils::addKV(target, "metadata",
                      JSON::toJSONString(serviceInfo2.getMetadata()));
  }
}

bool NamingProxy::createServiceInfo(const ServiceInfo2 &serviceInfo2,
...
bool NamingProxy::updateServiceInfo(const ServiceInfo2 &serviceInfo2,
                                    naming::Selector *selector)
    NACOS_THROW(NacosException) {
  list<NacosString> params;
  assembleServiceInfoRequest(params, serviceInfo2);
...
bool NamingProxy::serverHealthy() {
  ...
  NacosString result =
      reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                 UtilAndComs::NACOS_URL_BASE + "/operator/metrics",
             params, IHttpCli::GET);
...
Instance NamingProxy::getServiceInstance(
    const NacosString &serviceName, const NacosString &ip, int port,
  ...
  if (params.count(NamingConstant::NAMESPACE_ID) == 0) {
    ParamUtils::addKV(paramsList, NamingConstant::NAMESPACE_ID,
                      getNamespaceId());
  }

  if (params.count(NamingConstant::GROUP_NAME) == 0) {
    ParamUtils::addKV(paramsList, NamingConstant::GROUP_NAME,
                      ConfigConstant::DEFAULT_GROUP);
  }

  NacosString result =
      reqAPI("/" + _objectConfigData->_appConfigManager->getContextPath() +
                 UtilAndComs::NACOS_URL_BASE + "/instance",
             paramsList, IHttpCli::GET);

  return JSON::Json2Instance(result);
}

bool NamingProxy::updateServiceInstance(const Instance &instance)
    NACOS_THROW(NacosException) {
  list<NacosString> params;
  ...
  if (NacosStringOps::isNullStr(instance.namespaceId)) {
    ...
    ParamUtils::addKV(params, NamingConstant::NAMESPACE_ID,
                      instance.namespaceId);
  }

  if (!NacosStringOps::isNullStr(instance.clusterName)) {
    ParamUtils::addKV(params, NamingConstant::CLUSTER_NAME,
                      instance.clusterName);
  }

  if (instance.metadata.size() > 0) {
    ParamUtils::addKV(params, "metadata",
                      JSON::toJSONString(instance.metadata));
  }
  ParamUtils::addKV(params, "ephemeral",
                    NacosStringOps::valueOf(instance.ephemeral));
