...
namespace nacos {
static size_t receiveResponseCallback(void *contents, size_t size, size_t nmemb,
                                      void *userp) {
...
static size_t receiveHeaderCallback(void *contents, size_t size, size_t nmemb,
                                    void *userp) {
  ...
  if (pos != NULL) // Skip status
  {
    std::map<NacosString, NacosString> *respheaders =
        (std::map<NacosString, NacosString> *)userp;
...
CURL *HTTPCli::getCurlHandle() {
  CURL *curlHandle = (CURL *)pthread_getspecific(pthreadKey);
  if (curlHandle == NULL) {
...
NacosString HTTPCli::encodingParams(list<NacosString> &params) {
  ...
  for (list<NacosString>::iterator it = params.begin(); it != params.end();
       it++) {
...
NacosString HTTPCli::encodingParams(map<NacosString, NacosString> &params) {
  NacosString encodedParms = "";
  for (map<NacosString, NacosString>::iterator it = params.begin();
       it != params.end(); it++) {
    if (encodedParms.compare("") != 0) {
      encodedParms.append("&");
    }
...
void HTTPCli::assembleHeaders(list<NacosString> &assembledHeaders,
                              list<NacosString> &headers) {
  for (list<NacosString>::iterator it = headers.begin(); it != headers.end();
       it++) {
...
HttpResult HTTPCli::httpGet(const NacosString &path, list<NacosString> &headers,
                            list<NacosString> &paramValues,
                            const NacosString &encoding, long readTimeoutMs)
...
HttpResult
HTTPCli::httpGetInternal(const NacosString &path, list<NacosString> &headers,
  ...
  */
  ...
  for (list<NacosString>::iterator it = assembledHeaders.begin();
       it != assembledHeaders.end(); it++) {
...
We convert it into sth like this:
        foo=bar&bax=lol
*/
HttpResult HTTPCli::httpPost(const NacosString &path,
                             list<NacosString> &headers,
                             list<NacosString> &paramValues,
                             const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
...
HttpResult
HTTPCli::httpPostInternal(const NacosString &path, list<NacosString> &headers,
                          const NacosString &paramValues,
                          const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
  CURL *curlHandle = getCurlHandle();
...
We convert it into sth like this:
        foo=bar&bax=lol
*/
HttpResult HTTPCli::httpPut(const NacosString &path, list<NacosString> &headers,
                            list<NacosString> &paramValues,
                            const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
  NacosString parmVal;
...
HttpResult
HTTPCli::httpPutInternal(const NacosString &path, list<NacosString> &headers,
                         const NacosString &paramValues,
                         const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
  CURL *curlHandle = getCurlHandle();
...
HttpResult HTTPCli::httpDelete(const NacosString &path,
                               list<NacosString> &headers,
                               list<NacosString> &paramValues,
                               const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
...
HttpResult
HTTPCli::httpDeleteInternal(const NacosString &path, list<NacosString> &headers,
                            const NacosString &paramValues,
                            const NacosString &encoding, long readTimeoutMs)
    NACOS_THROW(NetworkException) {
  CURL *curlHandle = getCurlHandle();
...
void HTTPCli::HTTP_GLOBAL_INIT() { curl_global_init(CURL_GLOBAL_ALL); }

void HTTPCli::HTTP_GLOBAL_DEINIT() { curl_global_cleanup(); }
} // namespace nacos
