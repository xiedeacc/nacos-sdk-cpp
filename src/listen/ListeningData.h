...
public:
  NacosString toString() const {
    return tenant + ":" + dataId + ":" + group + ":" + dataMD5 +
           ", listenerList size=" +
           NacosStringOps::valueOf(listenerList.size());
  }

  NacosString getDataId() const { return dataId; }

  NacosString getTenant() const { return tenant; }

  NacosString getGroup() const { return group; }

  NacosString getMD5() const { return dataMD5; }

  void setMD5(const NacosString &md5) { dataMD5 = md5; }
  ...
  bool isEmpty() const { return listenerList.size() == 0; }

  ListeningData(const NacosString &tenant, const NacosString &dataId,
                const NacosString &group, const NacosString &cfgcontent) {
    setContent(tenant, dataId, group, cfgcontent);
  }

  void setContent(const NacosString &tenant, const NacosString &dataId,
                  const NacosString &group, const NacosString &cfgcontent) {
    this->tenant = tenant;
    this->dataId = dataId;
    this->group = group;
    if (!NacosisNull(cfgcontent)) {
      MD5 md5; // TODO:optimize this to a static object to avoid repeatedly
               // ctor&dtor of object
