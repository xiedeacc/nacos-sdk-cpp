...
class GroupKey {
public:
  static NacosString getKey(const NacosString &dataId,
                            const NacosString &group) {
    // DataID+GroupId
    return urlencode(dataId) + "+" + urlencode(group);
  }

  static NacosString getKeyTenant(const NacosString &dataId,
                                  const NacosString &group,
  ...
  /*static NacosString getKey(const NacosString &dataId, const NacosString
  &group, const NacosString &datumStr)
  {
      ...
      for (int i = 0; i < groupKey.length(); ++i) {
          ...
          if ('+' == c) {
              if (null == dataId) {
                  ...
                  throw new IllegalArgumentException("invalid groupkey:" +
  groupKey);
              }
          } else if ('%' == c) {
